...
start-Logging
$exportCSV = "targetTeamsToCreate-$(get-date -Format yyMMddHHmm).csv"
...
foreach($team in $teamsList) {
    ...
    write-log "processing ""$tName""..."
    $testTeam=Get-Team -DisplayName $tName
    if($NULL -eq $testTeam) {
        $team.'Target Team Name' = '<NOT FOUND>'
        write-log """$tName"" not found" -type warning
        continue
    }
