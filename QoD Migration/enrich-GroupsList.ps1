...
$header=@('Group Name','Target Group Name','Source MailNickname')
...
$groupsList|Add-Member -MemberType NoteProperty -Name 'objectID' -Value '' #QoD is case sensitive and need to have exactly 'objectID'
$groupsList|Add-Member -MemberType NoteProperty -Name 'target objectID' -Value ''
$groupsList|Add-Member -MemberType NoteProperty -Name 'target mailNickName' -Value ''

foreach($group in $groupsList) {
    $gName=$group.'Group Name'
    write-log "processing ""$gName""..."
    try {
        $testGroup=Get-UnifiedGroup -identity $gName
    } catch {
    ...
    $group.'objectID'=$testGroup.groupID
